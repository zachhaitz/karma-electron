...
function createPattern(filepath) {
  return {pattern: filepath, included: true, served: true, watched: false};
}
...
function ElectronFramework(files) {
  ...
  var nodeIntegrationFilepath = path.join(__dirname, 'node-integration-iframe.js');
  files.unshift(createPattern(nodeIntegrationFilepath));
}
