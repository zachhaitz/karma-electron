...
var $inject = ['config.basePath', 'config.customContextFile', 'config.customDebugFile'];
function createElectronPreprocessor(karmaBasePath, karmaCustomContextFile, karmaCustomDebugFile) {
  ...
  function electronPreprocessor(content, file, done) {
    ...
    var output = template({
      ...
      // TODO: Resolve loadScriptsViaRequire from config
