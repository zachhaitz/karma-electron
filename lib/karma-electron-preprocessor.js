...
var CONTENT_PLACEHOLDER_STR = '$$CONTENT$$';
...
function createElectronPreprocessor(karmaBasePath, karmaClientConfig, karmaCustomContextFile, karmaCustomDebugFile) {
  ...
  function electronPreprocessor(content, file, done) {
    ...
    // Otherwise, we are directly injecting content
      ...
      // Generate a mapping for our template
      // https://github.com/floridoo/concat-with-sourcemaps/blob/v1.0.5/index.js#L84-L97
      var outputSourceMapGenerator = new SourceMapGenerator();
      var contentIndex = output.indexOf(CONTENT_PLACEHOLDER_STR);
      assert.notEqual(contentIndex, -1);
      outputSourceMapGenerator.addMapping({
        original: {
          line: 1,
          column: 0
        },
      ...
      // Inject our content
      output = output.replace(CONTENT_PLACEHOLDER_STR, content);

      // // If there was a source map, add it back
      // TODO: Test out passed through content
      // if (contentSourceMap) {
      // }
      // Add on our mapping
      var outputSourceMap = convertSourceMap.fromObject(outputSourceMapGenerator);
      output += '\n' + outputSourceMap.toComment();
    }
